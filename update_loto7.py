...
# =========================================================
# 🧠 最新AIアルゴリズム（特徴量追加・動的重み付け・Optuna・5000パターン抽出）
# =========================================================
import numpy as np
try:
    import optuna
...
def generate_hybrid_predictions(X_train, y_train, X_latest):
    ...
    # ★修正：37個の数字それぞれの「出る確率(クラス1)」を取得する
    rf_preds = rf_model.predict_proba(X_latest)
    prob_rf = np.zeros(38)
    for i in range(37):
    ...
    predictions = []
    num_probs = {n: final_prob[n] for n in range(1, 38)}
    ...
    candidates = []
    for _ in range(5000):
        cand = list(np.random.choice(numbers_pool, size=7, replace=False, p=weights))
        cand.sort()
...
<body>
    <header>
        <!-- 詳細ページは /archive/ フォルダに入るので、画像やリンクには ../ を付けます -->
        <a href="../index.html" style="text-decoration: none;">
            <img src="../Lotologo001.png" alt="ロト＆ナンバーズ攻略局🎯完全無料のAI予想" style="max-width: 100%; height: auto; max-height: 180px;">
...
"""
    ...
    # ① 木曜の朝 (明日の予告) ※YouTubeの告知は別ファイルのためスキップ
    if today_weekday == 3 and current_hour < 19:
        x_send_flag = True
    ...
    if sns_send_flag and sns_msg:
        ...
        try:
            ...
            video_url = upload_video_to_cloudinary("reel_loto7.mp4")
            if video_url:
            ...
            post_to_tiktok("reel_loto7.mp4", caption)
...
if __name__ == "__main__":
    ...
    print("✨ [自動取得・完全決着版] ロト7 の自動更新とXへのポストが完了しました！")
